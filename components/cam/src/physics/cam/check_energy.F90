...
!------------------------------Arguments--------------------------------
    ...
    type(tp_stat_smry),optional,intent(inout) :: chunk_smry(:)
...
!!     te(i) = se(i) + ke(i) + (latvap+latice)*wv(i) + latice*wl(i)
    ...
    ! error checking
    !----
    call get_smry_field_idx('TOT_ENERGY_REL_ERR','check_energy_chng__'//trim(name),istat)
    ...
    if (print_energy_errors .and. name.eq.'clubb_tend') then
