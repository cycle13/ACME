...
!
! Added Thermal-Hydrology coupling subroutines: 04/2017
!----------------------------------------------------------------------------------------------
...
!     call pflotranModelGetTopFaceArea(pflotran_m)
  ...
  !-----------------------------------------------------------------------
    ...
    if (pf_tmode) then
        call get_clm_bceflx(bounds, filters, ifilter)
    ...
    if (pf_hmode) then      !if coupled 'H' mode between CLM45 and PFLOTRAN
        call get_clm_bcwflx(bounds, filters, ifilter)
    ...
    if (pf_hmode) then
        ...
        call update_soil_moisture_pf2clm(bounds, filters, ifilter)
        ...
        call update_bcflow_pf2clm( bounds, filters, ifilter)

    endif

    if (pf_tmode) then
        call pflotranModelGetTemperatureFromPF( pflotran_m )  ! thermal states
        call update_soil_temperature_pf2clm(bounds, filters, ifilter)
    endif
#endif
    ...
    if (pf_cmode) then
        ...
        call update_bgc_bcflux_pf2clm(clm_bgc_data, bounds, filters, ifilter)
...
#include "petsc/finclude/petscvec.h90"
    ...
    associate( &
         ...
         cactive    =>  col%active     , & !  [logic (:)]  column active or not
         !
         z          =>  col%z          , & !  [real(r8) (:,:)]  layer depth (m) (sort of centroid from surface 0 )
         zi         =>  col%zi         , & !  [real(r8) (:,:)]  layer interface depth (m)
...
!     PetscScalar, pointer :: zsoi_clm_loc(:)
    ...
    associate( &
         ...
         kd_decomp_pools          => clm_bgc_data%decomp_k_pools              , &
...
!-----------------------------------------------------------------------------
  ...
  !
  ! !INTERFACE:
  subroutine get_clm_bcwflx(bounds, filters, ifilter)
  !
  ! !DESCRIPTION:
  !
  ...
  ! !USES:
    ...
    use clm_varpar      , only : nlevsoi, nlevgrnd, max_pft_per_col
...
#include "petsc/finclude/petscvec.h90"
  ...
  !-----------------------------------------------------------------------
    ...
    t_grnd            => ces%t_grnd                              , &! ground surface temperature [K]
    t_soisno          => ces%t_soisno                            , &! column vertically-resolved temperature (snow-soil) [K]
    bsw               => cps%bsw                                 , &! Clapp and Hornberger "b" (nlevgrnd)
    hksat             => cps%hksat                               , &! hydraulic conductivity at saturation (mm H2O /s) (nlevgrnd)
    watsat            => cps%watsat                              , &! volumetric soil water at saturation (porosity) (nlevgrnd)
    sucsat            => cps%sucsat                              , &! minimum soil suction (mm) (nlevgrnd)
    watmin            => cps%watmin                              , &! restriction for min of volumetric soil water, or, residual vwc (-) (nlevgrnd)
    sucmin            => cps%sucmin                              , &! restriction for min of soil potential (mm) (nlevgrnd)
    frac_sno          => cps%frac_sno_eff                        , &! fraction of ground covered by snow (0 to 1)
    frac_h2osfc       => cps%frac_h2osfc                         , &! fraction of ground covered by surface water (0 to 1)
    qflx_top_soil     => cwf%qflx_top_soil                       , &! (column-level) net water input into soil from top (mm/s)
    qflx_tran_veg_col => pwf_a%qflx_tran_veg                     , &! column-level p-aggregated vegetation transpiration (mm H2O/s) (+ = to atm)
    qflx_ev_h2osfc_col=> pwf_a%qflx_ev_h2osfc                    , &! non-urban column-level p-aggregated evaporation flux from h2osfc (mm H2O/s) [+ to atm]
    qflx_ev_soil_col  => pwf_a%qflx_ev_soil                      , &! non-urban column-level p-aggregated evaporation flux from soil (mm H2O/s) [+ to atm]
    qflx_ev_snow_col  => pwf_a%qflx_ev_snow                      , &! non-urban column-level p-aggregated evaporation flux from snow (mm H2O/s) [+ to atm]
    h2osoi_liq        => cws%h2osoi_liq                          , &! liquid water (kg/m2)
    h2osoi_ice        => cws%h2osoi_ice                          , &! ice lens (kg/m2)
    rootr_pft         => pps%rootr                               , &! pft-level effective fraction of roots in each soil layer
    ...
    rootr_col         => cps%rootr_column                          &! effective fraction of roots in each soil layer
    )
...
!----------------------------------------------------------------------------------------------------------
#if 0
    ! (1) Computing the net sink of qflx into soil top 'qflx_top_soil' (originally those are in 'Hydrology2Mod.F90')
    ! note: 'SnowWater' calling adds the snow-melt water into the 'qflx_rain_grnd', which is calculated in 'bgp1'
    ! so, here what to do is to obtain updated 'cwf%qflx_top_soil', which also updated with snow evaporation/submission

    ! (1) snow hydrology (will by-pass in Hydrology2Mod.F90, if coupled with PFLOTRAN)
    call BuildSnowFilter(bounds, filters(ifilter)%num_nolakec, filters(ifilter)%nolakec, &
                                 filters(ifilter)%num_snowc,   filters(ifilter)%snowc,   &
                                 filters(ifilter)%num_nosnowc, filters(ifilter)%nosnowc)
    call SnowWater(bounds, filters(ifilter)%num_snowc,   filters(ifilter)%snowc,         &
                           filters(ifilter)%num_nosnowc, filters(ifilter)%nosnowc)

    ! (2) soil surface evaporation: needs further checking here -

    call p2c (bounds, filters(ifilter)%num_nolakec,    &  ! this may not be needed, but just in case
                      filters(ifilter)%nolakec, &
    ...
    do fc = 1, filters(ifilter)%num_soilc
       ...
       if (ltype(clandunit(c)) == istsoil .or. ltype(clandunit(c))==istcrop) then
           ! NOTE: 'qflx_evap_soi' is for urban col.
           !       'qflx_ev_snow/soi/h2osfc' are for non-urban col
           ! all of those 4 variables are calculated in 'BareGroundFluxesMod', 'CanopyFluxesMod',
           ! and then adjusted in 'Biogeophysics2' after soil temperature calls
           ! pft-level values are aggregated to col-level in clm_drvier.F90 by calling 'pft2col'.
           ! note that: all 4 variables could be negative (i.e., dew formation on ground)

           qflx_evap(c)=(1.0_r8 - frac_sno(c) - frac_h2osfc(c))*qflx_ev_soil_col(c)
       end if

    end do

    ! (3) Compute the vegetation Transpiration
    !  (originally those are in 'SoilHydrologyMod.F90': SoilWater subroutine, which bypassed when coupled with PF)
    !do j = 1, nlevsoi
        do fc = 1, filters(ifilter)%num_soilc
            c = filters(ifilter)%soilc(fc)
            rootr_col(c,:) = 0._r8              ! initializing to zero for ALL layers ('nlevgrnd')
        end do
    !end do
    temp(:) = 0._r8       !

    do pftindex = 1,max_pft_per_col
       do j = 1,nlevgrnd !nlevsoi
           do fc = 1, filters(ifilter)%num_soilc
               c = filters(ifilter)%soilc(fc)
...
!----------------------------------------------------------------------------------------------------------
    ...
    do fc = 1, filters(ifilter)%num_soilc
      ...
      do j = 1, clm_pf_idata%nzclm_mapped
        if(j<=nlevgrnd) then
          ...
          qflwt_clmp_loc(cellcount ) = forc_t(gcount+1) - tfrz
       ...
       do j = 1, clm_pf_idata%nzclm_mapped
         ...
         if (j<=nlevgrnd) then
          ...
          sr = 1.01_r8*sr_pcwmax_clms_loc(cellcount) * &              ! '1.01' will give 1% for hard-accessible pore and holding error in the calculation
                    porosity_clms_loc(cellcount)!watsat(c,j)          ! PF saturation ==> 'real' vwc
          ...
          if (j .eq. 1) then
             ...
             if (.not.shr_infnan_isnan(qflx_evap(c))) then
               ...
               if( qflx_evap(c)>0._r8) then
                  ...
                  qflx_ev_soil_col(c) = qflx_ev_soil_col(c) * reductor  ! NOTE: this is needed for adjusting both Water and Energy flux later on
               endif
          ...
          qflx = -qflx_tran_veg_col(c)*rootr_col(c,j)*area*1.e-3*denh2o
  ...
  !
  ! !INTERFACE:
  subroutine get_clm_bceflx(bounds, filters, ifilter)
  !
  ! !DESCRIPTION:
  !
...
#include "petsc/finclude/petscvec.h90"
  ...
  ! !LOCAL VARIABLES:
    ...
    real(r8) :: t_nearsurf_col(bounds%begc:bounds%endc)
    real(r8) :: fact_temp(bounds%begc:bounds%endc, -nlevsno+1:nlevgrnd)
    real(r8) :: c_h2osfc_temp(bounds%begc:bounds%endc)
    real(r8) :: xmf_h2osfc_temp(bounds%begc:bounds%endc)
  ...
  !-----------------------------------------------------------------------
    associate ( &
    cgridcell         => col%gridcell                            , &! column's gridcell
    dz                => cps%dz                                  , &! layer thickness depth (m)
    snl               => cps%snl                                 , &! number of snow layers (negative)
    frac_sno_eff      => cps%frac_sno_eff                        , &! fraction of ground covered by snow (0 to 1)
    frac_h2osfc       => cps%frac_h2osfc                         , &! fraction of ground covered by surface water (0 to 1)
    htvp              => cps%htvp                                , &! latent heat of vapor of water (or sublimation) [j/kg]
    eflx_fgr0_snow    => cef%eflx_fgr0_snow                      , &! heat flux from snow column (W/m**2) [+ = into soil]
    eflx_fgr0_h2osfc  => cef%eflx_fgr0_h2osfc                    , &! heat flux from surface water column (W/m**2) [+ = into soil]
    eflx_fgr0_soil    => cef%eflx_fgr0_soil                      , &! heat flux from near-surface air (W/m**2) [+ = into soil]
    eflx_rnet_soil    => cef%eflx_rnet_soil                      , &! heat flux between soil layer 1 and above-air, excluding SH and LE (i.e. radiation form) (W/m2) [+ = into soil]
    eflx_bot          => cef%eflx_bot                            , &! heat flux from beneath column (W/m**2) [+ = upward]
    t_soisno          => ces%t_soisno                            , &! snow-soil layered temperature [K]
    t_h2osfc          => ces%t_h2osfc                            , &! surface-water temperature [K]
    t_nearsurf        => pes%t_nearsurf                          , &! mixed air/veg. temperature near surface (for coupling with PFLOTRAN as BC)
    qflx_ev_soil_col  => pwf_a%qflx_ev_soil                       &! non-urban column-level p-aggregated evaporation flux from soil (mm H2O/s) [+ to atm]
    )
...
#if 0
    ! (1a) aggregating patch-level (which saved in CLM) to column-level
    !      for near-surface air/veg. mixed temperature
    call p2c (bounds, filters(ifilter)%num_nolakec,            &
                      filters(ifilter)%nolakec,                &
                      t_nearsurf(bounds%begp:bounds%endp),     &
                      t_nearsurf_col(bounds%begc:bounds%endc))

    ! (1b) snow/surface-water thermal-hydrology
    !  note: the following module only runs snow/surface-water portion (1st soil as bottom BC) for SOIL columns
    !        BUT will runs normally for other column-types
    ! ( this will bring ALL above-soil processes updated currently before PFLOTRAN calling,
    !   otherwise, relevant variables could be at previous-timestep)

    call SoilTemperature(bounds, filters(ifilter)%num_urbanl,     &
                                 filters(ifilter)%urbanl,         &
                                 filters(ifilter)%num_nolakec,    &
                                 filters(ifilter)%nolakec,        &
                                 xmf_temp(bounds%begc:bounds%endc),      &   ! don't need these here really
                                 fact_temp(bounds%begc:bounds%endc, :),  &
                                 c_h2osfc_temp(bounds%begc:bounds%endc), &
                                 xmf_h2osfc_temp(bounds%begc:bounds%endc))
#endif
!----------------------------------------------------------------------------------------------------------
    ! (2) pass the clm_gflux/gtemp to the vec
       ...
       if (snl(c) < 0) then
          if(frac_h2osfc(c) /= 0._r8) then
             t_grnd0 = frac_sno_eff(c) * t_soisno(c,0)                                   &   ! a note here: 't_soisno(c,0)' NOT always has a meanful value
                   + (1.0_r8 - frac_sno_eff(c) - frac_h2osfc(c)) * t_nearsurf_col(c)     &
                   + frac_h2osfc(c) * t_h2osfc(c)                                            ! a note here: 't_h2osfc' NOT always has a meanful value
          else
             t_grnd0 = frac_sno_eff(c) * t_soisno(c,0)                           &
                   + (1.0_r8 - frac_sno_eff(c)) * t_nearsurf_col(c)

          endif
       else
          if(frac_h2osfc(c) /= 0._r8) then
             t_grnd0 = (1.0_r8 - frac_h2osfc(c)) * t_nearsurf_col(c)                     &
                   + frac_h2osfc(c) * t_h2osfc(c)
          else
             t_grnd0 = t_nearsurf_col(c)
          endif
       endif

       gtemp_subsurf_clmp_loc(gcount+1)  = t_grnd0 - tfrz
       gtemp_subbase_clmp_loc(gcount+1)  = -9999                                ! not yet get it from CLM (i.e.,dirichlet type not available)

       ! soil evaporation adjusted by liq. water available in the first soil layer
       eflx_ev0 = -qflx_ev_soil_col(c)*htvp(c)*(1.0_r8 - frac_sno_eff(c) - frac_h2osfc(c))  ! - = LE out of soil

       ! net radiation into soil
       eflx_rnet0 = (1.0_r8 - frac_sno_eff(c) - frac_h2osfc(c))   &
                  * eflx_rnet_soil(c)

       ! net heat flux into soil (all forms)
  ...
  !BOP
  !
  ! !ROUTINE: get_clm_bgc_conc(clm_bgc_data, bounds, filters, ifilter)
  !
  ! !INTERFACE:
...
#include "petsc/finclude/petscvec.h90"
    ...
    type(clm_bgc_interface_data_type), intent(in) :: clm_interface_data
...
#else
      gcount = g - bounds%begg                 ! 0-based
      if (mapped_gcount_skip(gcount+1)) cycle  ! skip inactive grid, but numbering
#endif
...
#include "petsc/finclude/petscvec.h90"
  ...
  !-----------------------------------------------------------------------
    associate ( &
       ...
       t_soisno        => clm_interface_data%t_soisno_col   &  ! snow-soil temperature (Kelvin) [:, 1:nlevgrnd]
       )
  ...
  !
  ! !INTERFACE:
  subroutine update_bcflow_pf2clm(bounds, filters, ifilter)
  !
  ! !DESCRIPTION:
  ...
  !
  ! !USES:
    use clmtype         , only : col, lun, cps, ces, cwf, pwf_a
    use clm_varcon      , only : istsoil, istcrop, denh2o, tfrz
    use clm_time_manager, only : get_step_size, get_nstep
...
#include "petsc/finclude/petscvec.h90"
    ...
    t_grnd            =>    ces%t_grnd            , & ! ground surface temperature [K]
    zi                =>    cps%zi                , & ! Input: (:,:) soil layer interface depth (m)
    dz                =>    cps%dz                , & ! Input: (:,:) soil layer thickness (m)
    frac_sno          =>    cps%frac_sno_eff      , & ! Input: fraction of ground covered by snow (0 to 1)
    frac_h2osfc       =>    cps%frac_h2osfc       , & ! Input: fraction of ground covered by surface water (0 to 1)
    qflx_top_soil     =>    cwf%qflx_top_soil     , & ! Input:  [real(r8) (:)]  net water input into soil from top (mm/s)
    qflx_ev_h2osfc    =>    pwf_a%qflx_ev_h2osfc  , & ! Input: column-level evaporation flux from h2osfc (mm H2O/s) [+ to atm]
    qflx_ev_soil      =>    pwf_a%qflx_ev_soil    , & ! Input: column-level evaporation flux from soil (mm H2O/s) [+ to atm]
    qflx_evap_soi_col =>    pwf_a%qflx_evap_soi   , & ! Input: column-level soil evaporation (mm H2O/s) (+ = to atm)
    qflx_ev_snow      =>    pwf_a%qflx_ev_snow    , & ! Input: column-level evaporation flux from snow (mm H2O/s) [+ to atm]
    qflx_surf         =>    cwf%qflx_surf         , & ! Output: [real(r8) (:)]  surface runoff (mm H2O /s)
    qflx_infl         =>    cwf%qflx_infl         , & ! Output: [real(r8) (:)]  soil infiltration (mm H2O /s)
    qflx_drain        =>    cwf%qflx_drain          & ! Output: [real(r8) (:)]  sub-surface runoff (drainage) (mm H2O /s)
    )
    ...
    do fc = 1, filters(ifilter)%num_soilc
       ...
       if (ltype(clandunit(c)) == istsoil .or. ltype(clandunit(c))==istcrop) then
           ...
           ! note that: all 4 variables could be negative (i.e., dew formation on ground)
           qflx_evap(c)=(1.0_r8 - frac_sno(c) - frac_h2osfc(c))*qflx_ev_soil(c) + &
          ...
          qflx_evap(c) = (1.0_r8 - frac_sno(c))*qflx_ev_soil(c)
       end if
    ...
    do fc = 1, filters(ifilter)%num_soilc
      ...
      if (t_grnd(c) > tfrz) then   ! frozen ground, no dew contribution to subsurface infiltration
        if (ltype(clandunit(c)) == istsoil .or. ltype(clandunit(c))==istcrop) then
           dew = -min(0._r8, (1.0_r8 - frac_sno(c) - frac_h2osfc(c))*qflx_ev_soil(c) + &
                             frac_h2osfc(c)*qflx_ev_h2osfc(c))
        else
           dew = -min(0._r8, qflx_evap_soi_col(c))
        end if
      endif
...
! ------------------------------------------------------------------------
  ...
  !
  subroutine update_bgc_bcflux_pf2clm(clm_bgc_data, bounds, filters, ifilter)
